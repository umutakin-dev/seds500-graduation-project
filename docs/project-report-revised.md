...
**Advisor:** Asst. Prof. Dr. Damla Oğuz (IZTECH)
...
| **Improvement**      | TabDDPM-style techniques                          | 87-98% of baseline performance            |
| **Privacy**          | Unknown if safe                                   | Validated: AUC=0.51 (no leak)             |
| **vs Alternatives**  | CTGAN, SMOGN untested                             | TabDDPM beats both significantly          |
| **Generalization**   | Single dataset tested                             | Validated on 2 datasets (Ozel, Production)|
...
2. **Augmentation didn't improve accuracy, but revealed a quality difference.** While testing augmentation methods, we observed that models trained on SMOGN-augmented data sometimes completely failed (R² went negative), whereas models trained on diffusion-augmented data maintained performance. This wasn't about improving accuracy; it was about data quality.
...
- **q(·)**: The forward process distribution that adds noise to data
- **p_θ(·)**: The learned reverse process with neural network parameters θ
- **x_t**: The data at timestep t (x₀ is clean data, x_T is pure noise)
...
Here, μ_θ(x_t, t) is the mean predicted by our neural network given noisy data x_t and timestep t. The model learns to predict the original clean data x₀ from noisy observations.
...
The replacement scenario is the most challenging test: training a model entirely on synthetic data and evaluating on real test data. This measures how well the synthetic data captures the true data distribution. Table 3 shows that TabDDPM-style diffusion dramatically outperforms all other methods.
...
**Table 3b: Production Dataset Results**
...
**Key insight:** The higher baseline R² (0.994 vs 0.645) indicates stronger patterns in the Production data, which may be easier for diffusion to capture. However, the absolute performance (R² = 0.9785 for replacement) is excellent regardless.
...
![Figure 6: Key Results Summary - TabDDPM achieves high utility with strong privacy](figures/fig9_key_results_summary.png)
_Figure 6: Summary of key results showing 87-98% utility retention across datasets, 0.51 privacy AUC (equivalent to random guessing), and 3.3x improvement over simple diffusion._
...
[1] Sweeney, L. (2000). Simple Demographics Often Identify People Uniquely. Carnegie Mellon University, Data Privacy Working Paper 3.
...
[4] Branco, P., Torgo, L., & Ribeiro, R. P. (2017). SMOGN: A Pre-processing Approach for Imbalanced Regression. PKDD 2017.

[5] Xu, L., et al. (2019). Modeling Tabular Data using Conditional GAN. NeurIPS 2019.

[6] Kotelnikov, A., Barber, D., & Zohren, S. (2023). TabDDPM: Modelling Tabular Data with Diffusion Models. ICML 2023. _[200+ citations as of 2025]_

[7] Kim, J., Lee, C., & Park, N. (2023). STaSy: Score-based Tabular Data Synthesis. ICLR 2023.

[8] Zhang, H., et al. (2024). Mixed-Type Tabular Data Synthesis with Score-based Diffusion in Latent Space. ICLR 2024.

[9] Shokri, R., et al. (2017). Membership Inference Attacks Against Machine Learning Models. IEEE S&P 2017.

[10] Nichol, A., & Dhariwal, P. (2021). Improved Denoising Diffusion Probabilistic Models. ICML 2021.

[11] Jang, E., Gu, S., & Poole, B. (2017). Categorical Reparameterization with Gumbel-Softmax. ICLR 2017.
