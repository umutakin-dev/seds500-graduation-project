"""
Prepare ozel data for HybridDiffusion - V3 (TabDDPM-style preprocessing).

Key changes from v1/v2:
- NO clipping to [-1, 1] - keep unbounded Gaussian from QuantileTransformer
- This matches TabDDPM's approach and should fix model collapse

Features:
- Cap (diameter): numeric
- Boy (length): numeric
- IslemTipi, AnmaOlcusu, SartnameSimple, UY: categorical

Target: MakineSure (Makine Süre 100.000 ADET DK)
"""
...
def prepare_data(df, test_size=0.2):
    """
    Prepare data for HybridDiffusion - V3 (TabDDPM-style).

    KEY DIFFERENCE: No clipping to [-1, 1]. Keep unbounded Gaussian.
    """
    print("\n" + "=" * 80)
    print("V3: TABDDPM-STYLE PREPROCESSING (UNBOUNDED GAUSSIAN)")
    print("=" * 80)
    ...
    # Combine: [Cap, Boy, target] + [categoricals as indices]
    ...
    print(f"\nFinal shapes:")
    print(f"  Train numeric: {X_train_full.shape}")
    print(f"  Cat cardinalities: {cat_cardinalities}")

    return {
        "X_train_num": X_train_full,  # Scaled numeric only
        "X_test_num": X_test_full,
        "X_train_num_unscaled": X_num_train,  # Original for ML
        "X_test_num_unscaled": X_num_test,
        "y_train": y_train,  # Original for ML
        "y_test": y_test,
        "cat_train": cat_train,
        "cat_test": cat_test,
...
def main():
    ...
    parser.add_argument("--output", type=str, default="data/ozel_rich_v3/prepared.pt")
