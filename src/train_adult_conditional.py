"""
Train class-conditional hybrid diffusion model on Adult dataset.

This script trains a model that can generate samples for a specific class,
enabling targeted augmentation for imbalanced datasets.

Usage:
    python src/train_adult_conditional.py --epochs 500 --device cuda --label_dropout 0.1
"""
...
def load_adult_data():
    ...
    print(f"Train labels: class 0 = {(y_train == 0).sum()}, class 1 = {(y_train == 1).sum()}")
...
def train(args):
    ...
    num_classes = 2  # Binary classification (<=50K vs >50K)
    ...
    model = HybridMLPDenoiser(
        ...
        dropout=0.1,
        num_classes=num_classes,  # Enable class conditioning
    ).to(args.device)
    ...
    # Training loop
    print(f"\nTraining CLASS-CONDITIONAL model on {args.device}...")
    print(f"Epochs: {args.epochs}, Batch size: {args.batch_size}, LR: {args.lr}")
    print(f"Label dropout: {args.label_dropout} (for classifier-free guidance)")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            # Training with class labels and label dropout for CFG
            losses = diffusion.training_loss(
                model, x_0, y=y, label_dropout=args.label_dropout
            )
            loss = losses["loss"]
    ...
    # Save checkpoint
    checkpoint_dir = Path("checkpoints/adult_conditional")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label_dropout", type=float, default=0.1,
                        help="Label dropout probability for classifier-free guidance")
